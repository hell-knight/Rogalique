...
{
	Creeper::Creeper(const MyEngine::Vector2Df& position, MyEngine::GameObject* player)
		: Character(position, player, "Creeper")
	{
		// using an AI texture for demonstration purposes (you can replace it with “creeper”)
		auto* tex = MyEngine::ResourceSystem::Instance()->GetTextureMapElementShared("ai", 0);
		if (!tex) {
			LOG_ERROR("Creeper: failed to get texture for AI, object will have no texture.");
		...
		auto health = gameObject->AddComponent<MyEngine::HealthComponent>(gameObject, 50.f, 0.f);
		...
		auto attack = gameObject->AddComponent<MyEngine::AttackComponent>(gameObject, 15.f, 120.f, 1.5f);
