...
SpriteAnimationComponent::SpriteAnimationComponent(GameObject* gameObject)
    ...
    if (!transform || !renderer) {
        LOG_ERROR(
            "SpriteAnimationComponent needs Transform and SpriteRenderer. "
            "Removing.");
        gameObject->RemoveComponent(this);
    }
}

void SpriteAnimationComponent::Initialize(const std::string& textureMapName,
                                          float framerate) {
    ...
    for (int i = 0; i < frameCount; ++i)
        frames.push_back(ResourceSystem::Instance()->GetTextureMapElementShared(
            textureMapName, i));
    ...
    LOG_INFO("SpriteAnimationComponent initialized with " +
             std::to_string(frameCount) + " frames from " + textureMapName);
}
...
void SpriteAnimationComponent::Update(float deltaTime) {
    ...
    if (distance > threshold) {
        if (!isMoving) {
            // Once we start moving, we'll immediately switch to the second
            // frame (if available)
            timer = 0.f;
            currentFrame = 0;
            isMoving = true;
