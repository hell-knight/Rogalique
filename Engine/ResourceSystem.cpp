...
void ResourceSystem::LoadTextureMap(const std::string& name,
    ...
    if (!textureMap.loadFromFile(sourcePath)) {
        LOG_ERROR("Failed to load texture map '" + name + "' from" +
                  sourcePath);
    }
    ...
    for (unsigned int y = 0; y <= textureSize.y - elementPixelSize.y;
        ...
        for (unsigned int x = 0; x <= textureSize.x - elementPixelSize.x;
            ...
            if (newTextureMapElement->loadFromFile(
                ...
                LOG_WARN("Failed to extract element " +
                         std::to_string(loadedElements) +
    ...
    if (loadedElements == 0) {
        LOG_ERROR("No elements could be loaded for texture map '" + name +
                  "'. Map will be empty.");
    } else {
        LOG_INFO("Texture map '" + name + "' loaded with " +
                 std::to_string(loadedElements) + " elements (requested " +
...
const sf::Texture* ResourceSystem::GetTextureMapElementShared(
    ...
    if (elementIndex < 0 || elementIndex >= static_cast<int>(textures.size())) {
        LOG_ERROR("Texture map element index " + std::to_string(elementIndex) +
