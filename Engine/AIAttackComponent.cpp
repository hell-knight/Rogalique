...
namespace MyEngine
{
        ...
        : Component(gameObject)
    {
        ...
        {
            LOG_ERROR("AIAttackComponent: missing Transform or Attack component. Removing.");
    ...
    void AIAttackComponent::SetAttackRange(float range)
    {
        attackRange = range;
    }
    ...
    {
        if (!selfTransform || !targetTransform || !attack)
            return;

        Vector2Df diff = targetTransform->GetWorldPosition() - selfTransform->GetWorldPosition();
