...
namespace MyEngine
{
    ...
    /*void HealthComponent::SetMaxHealth(float maxHP)
    {
        maxHealth = maxHP;
        health = maxHP;
        LOG_INFO(gameObject->GetName() + " max health set to " + std::to_string(maxHealth));
    }
    ...
    void HealthComponent::UnsubscribeOnDeath(int id)
    {
        deathCallbacks.erase(id);
    }
}